...
def test_require_raises_when_gatk_absent(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="gatk not on PATH"):
        bam_calling.call_sample_vcf(
            tmp_path / "x.bam", tmp_path / "r.fasta", tmp_path / "w", sample_id="x"
        )
...
def _ped() -> Pedigree:
    return Pedigree(
        members=[Member(id="p", role=Role.proband, sex=Sex.male, affected=Affected.affected)]
    )
...
def test_align_fastq_paired_end(tmp_path, monkeypatch):
    ...
    r1 = tmp_path / "p_R1.fastq.gz"
    r1.write_bytes(b"")
    r2 = tmp_path / "p_R2.fastq.gz"
    r2.write_bytes(b"")
    ref = tmp_path / "ref.fasta"
    ref.write_bytes(b"")
...
def test_call_all_from_fastq_one_vcf_per_role(tmp_path, monkeypatch):
    ...
    for role in ("proband", "mother"):
        r1 = tmp_path / f"{role}_R1.fq.gz"
        r1.write_bytes(b"")
        r2 = tmp_path / f"{role}_R2.fq.gz"
        r2.write_bytes(b"")
        fq[role] = (r1, r2)
...
def test_run_case_fastq_without_reference_errors():
    ...
    with pytest.raises(ValueError, match="reference"):
        run_case(
            "c1", _ped(), fastq_paths={"p": (Path("p_R1.fq.gz"), Path("p_R2.fq.gz"))}, hpo_ids=[]
        )
...
def _install_fake_sv_tools(monkeypatch, tmp_path) -> list[list[str]]:
    ...
    def fake_run(cmd, check=False, **kw):  # noqa: ARG001
        ...
        if cmd and cmd[0] == "AnnotSV" and "-outputFile" in cmd:
            out = Path(cmd[cmd.index("-outputFile") + 1])
            out.parent.mkdir(parents=True, exist_ok=True)
            out.write_text("AnnotSV_ID\n")
...
def test_call_svs_from_bam_runs_gcnv_then_annotsv(tmp_path, monkeypatch):
    ...
    ref = tmp_path / "ref.fasta"
    ref.write_bytes(b"")
    intervals = tmp_path / "intervals.interval_list"
    intervals.write_bytes(b"")
    ploidy = tmp_path / "ploidy-model"
    ploidy.mkdir()
    cnv = tmp_path / "cnv-model"
    cnv.mkdir()
...
def test_build_gcnv_pon_runs_cohort_sequence_and_makes_bundle(tmp_path, monkeypatch):
    ...
    priors.write_bytes(b"")
    normals = []
    for i in range(3):
        b = tmp_path / f"normal_{i}.bam"
        b.write_bytes(b"")
        normals.append(b)
...
def test_build_pon_bundle_round_trips_through_extract(tmp_path, monkeypatch):
    ...
    ploidy = tmp_path / "ploidy-model"
    ploidy.mkdir()
    (ploidy / "x").write_text("p")
    cnv = tmp_path / "cohort-model"
    cnv.mkdir()
    (cnv / "y").write_text("c")
